...
export function ProductListClient({ initialProducts }: { initialProducts: any[] }) {
  // Gunakan SWR untuk otomatis nge-fetch / polling ke server di background
  const { data: products } = useSWR(
    ...
    {
      fallbackData: initialProducts, // Data awal dari Server Component (biar tidak ada loading state kosong)
      refreshInterval: 3000, // Akan nge-cek data baru tiap 3 detik kalau tab browser sedang aktif
      revalidateOnFocus: true, // Auto-update kalau user balik ke tab website kita
    }
  );
